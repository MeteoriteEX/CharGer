...
public class KnowledgeManager {
	...
	public   Action summarizeKnowledgeAction = new AbstractAction()
	{
		...
		{
			if ( s.equals( Action.NAME ) ) return charger.Global.strs( "SummarizeKnowledgeLabel" );
	...
	public  Graph[] getAllGraphs()
	{
		...
		for ( int k = 0; k < knowledgeSources.size(); k++ )
		{
			if ( knowledgeSources.get( k ) == null )
				charger.Global.error( "Knowledge source number " + k + " was null." );
    ...
    public  AbstractTupleMatcher createCurrentTupleMatcher()
    {
        ...
        className = charger.Global.matchingStrategy;
        ...
        catch ( ClassNotFoundException cnfe ) { charger.Global.error( "Class " + className + " not found." ); }
        catch ( InstantiationException ie )   { charger.Global.error( "Class " + className + " couldn't be instantiated." ); }
        catch ( IllegalAccessException iae )  { charger.Global.error( "Class " + className + " couldn't be accessed." ); }
        return matcher;
    }
