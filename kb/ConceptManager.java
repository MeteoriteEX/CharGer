...
public class ConceptManager {
    ...
    public static JTable getBinaryTupleTable( ConceptsToInclude selector ) {
        ...
        {
            readonlymodel = new DefaultTableModel( tableValues, kb.BinaryTuple.getTupleColumnLabels().toArray() ) {
...
//            readonlymodel = new DefaultTableModel( binaryRelationTuples, tupleColumnLabels ) {
    ...
    public static ArrayList getAll( GraphObject pattern ) {
        ...
        while ( graphiter.hasNext() ) {
            ...
            charger.obj.Graph g = ef.TheGraph;
    ...
    public static Vector getAllConcepts( ConceptsToInclude selector ) {
        ...
        while ( graphiter.hasNext() ) {
            ...
            while ( iter.hasNext() ) {
                ...
                charger.obj.Concept c = (charger.obj.Concept)iter.next();
                craft.Craft.say( "concept " + c.getTextLabel() + " accessed." );
                if ( selector == ConceptsToInclude.ALL || ( selector == ConceptsToInclude.GENERIC_ONLY && c.getReferent().equals( "" ) ) ) {
                    craft.Craft.say( "concept " + c.getTextLabel() + " included." );
        ...
        craft.Craft.say( nameGraphIDvect.size() + " concepts were gathered." );
     ...
     */
    public static ArrayList<BinaryTuple> getBinaryRelationTuples( charger.obj.Graph g, kb.ConceptManager.ConceptsToInclude selector ) {
     ...
     * @see #makeTableEntryArrayList
     */
    public static ArrayList<BinaryTuple> getBinaryRelationTuples( kb.ConceptManager.ConceptsToInclude selector ) {
    ...
    public static ArrayList<BinaryTuple> getAllRelationTuples( ConceptsToInclude selector ) {
        ...
        charger.obj.Graph[] allGraphs = Global.knowledgeManager.getAllGraphs();
        // for every available graph
        for ( int gnum = 0; gnum < allGraphs.length; gnum++ ) {
            charger.obj.Graph g = allGraphs[ gnum];
            {
                ...
                while ( nodeiter.hasNext() ) {
                    ...
                    if ( gn instanceof charger.obj.Relation || gn instanceof charger.obj.Actor ) {
                        ...
                        for ( int n1 = 0; n1 < edges.length; n1++ ) {
                            for ( int n2 = n1 + 1; n2 < edges.length; n2++ ) {
                                charger.obj.Concept c1 = null;
                                charger.obj.Concept c2 = null;
                                if ( edges[ n1].howLinked( rel ) == GEdge.Direction.FROM ) {
                                    c1 = (charger.obj.Concept)edges[ n1].toObj;
                                } else {
                                    c1 = (charger.obj.Concept)edges[ n1].fromObj;
                                }
                                if ( edges[ n2].howLinked( rel ) == GEdge.Direction.FROM ) {
                                    c2 = (charger.obj.Concept)edges[ n2].toObj;
                                } else {
                                    c2 = (charger.obj.Concept)edges[ n2].fromObj;
                                }
...
//                                ArrayList v = makeTableEntryArrayList( c1, rel, c2, selector );
    ...
    protected static ArrayList makeTableEntryArrayList(
            charger.obj.Concept c1, charger.obj.GNode rel, charger.obj.Concept c2, ConceptsToInclude selector ) {
    ...
    public static String binaryRelationTableEntryToHTML( ArrayList tableEntry ) {
        ...
        col = kb.ConceptManager.COL_CONCEPT_LABEL_1;
        html += " <TD>"
                + charger.xml.XMLGenerator.quoteForXML( tableEntry.get( col ).toString().trim() ) + "\n";

        col = kb.ConceptManager.COL_RELATION_LABEL;
        html += " <TD>"
                + charger.xml.XMLGenerator.quoteForXML( tableEntry.get( col ).toString().trim() ) + "\n";

        col = kb.ConceptManager.COL_CONCEPT_LABEL_2;
        html += " <TD>"
                + charger.xml.XMLGenerator.quoteForXML( tableEntry.get( col ).toString().trim() ) + "\n";
        return html;
...
//
     ...
     */
    private static boolean swapBinaryRelation( charger.obj.Concept c1, charger.obj.GNode rel, charger.obj.Concept c2 ) {
...
//                );
//    }
    public static String makeSentence( charger.obj.Concept c1, charger.obj.GNode rel, charger.obj.Concept c2 ) {
        if ( rel instanceof charger.obj.Relation ) {
            return Reporter.natLang( (charger.obj.Relation)rel, false );
        } else {
            return makeSentenceOLD( c1, rel, c2 );
        }
    }

    public static String makeSentenceOLD( charger.obj.Concept c1, charger.obj.GNode rel, charger.obj.Concept c2 ) {
        String c1label = null;
        String c2label = null;
        String rlabel = null;
        ...
        conn2 = Reporter.tmap.suffix( rlabel, rel instanceof charger.obj.Actor );
        conn1 = Reporter.tmap.prefix( rlabel, rel instanceof charger.obj.Actor );
     ...
     * turns up.
     */
    public static charger.gloss.AbstractTypeDescriptor[] getSensesFromPhrase( String phrase, charger.gloss.wn.WordnetManager wnmgr ) {
        ...
        craft.Craft.say( "===== getting senses for phrase of length " + words.size() );
        while ( wordnumber < words.size() && keepGoing ) {
            craft.Craft.say( "looking for previous senses starting at word " + wordnumber );
            // look for a previous descriptor for initial word(s)
            charger.gloss.AbstractTypeDescriptor descr = findLongestTermAlreadyDefined( words, wordnumber );
            if ( descr != null ) {
                craft.Craft.say( "initial descr type is " + descr.getClass() );
                String alreadyDefinedLabel = descr.getLabel();
                craft.Craft.say( "querying for sense with the label/phrase " + alreadyDefinedLabel + "/" + phrase );
            ...
            {
                craft.Craft.say( "Querying for terms starting at word " + words.get( wordnumber ) );
                ...
                for ( int trynum = words.size() - 1; trynum >= wordnumber; trynum-- ) {
                    ...
                    craft.Craft.say( "creating term " + newTerm + " of length " + len );
        ...
        return (charger.gloss.AbstractTypeDescriptor[])descriptors.toArray(new charger.gloss.AbstractTypeDescriptor[ 0 ] );
    }
    ...
    public static String makeTerm( ArrayList _words, int startWord, int endWord ) {
        craft.Craft.say( "makeTerm: from words " + startWord + " to " + endWord + " of " + _words );
        ...
        craft.Craft.say( "makeTerm: result is " + term.toString() );
     ...
     */
    public static charger.gloss.AbstractTypeDescriptor findLongestTermAlreadyDefined( ArrayList wordlist, int startWord ) {
        craft.Craft.say( "findLongestTermAlreadyDefined: start at word " + startWord + " of wordlist " + wordlist );
        // NOT WORKING!!
        for ( int wordnum = wordlist.size() - 1; wordnum >= startWord; wordnum-- ) {
            String term = makeTerm( wordlist, startWord, wordnum );
            charger.gloss.AbstractTypeDescriptor d[] = Global.knowledgeManager.findTypeDescriptor( term, false );
            craft.Craft.say( "findLongestTermAlreadyDefined: found " + d.length + " terms of length " + ( wordnum - startWord + 1 ) );
     ...
     */
    public static charger.obj.Graph makeTypeHierarchy() {
        charger.obj.Graph h = new charger.obj.Graph();
        charger.obj.Graph[] hierarchies = Global.knowledgeManager.findGraphsWithTypeHierarchy();
     ...
     */
    private static void fillInGlossaryDefinitions( charger.obj.Graph g ) {
        ...
        while ( iter.hasNext() ) // for each type label in the type hierarchy graph
        {
            ...
            if ( mydescr.length == 0 ) {
                charger.obj.Graph[] gs = Global.knowledgeManager.getAllGraphs();
                ...
                if ( mydescr.length == 0 ) // still no definition found
                {
                    ...
                    if ( mydescr.length == 0 ) {
                        charger.Global.info( "no definition found for " + type.getTextLabel() );
                    }
                }
            }
            if ( mydescr.length > 0 ) {
     ...
     */
    private static void addNewTypeLabels( charger.obj.Graph g ) {
        ...
        while ( gns.hasNext() ) {
            ...
            charger.obj.Concept c = (charger.obj.Concept)conceptVector.get( 2 );
     ...
     *
     */
    private static boolean okayToAddTypeLabel( charger.obj.Concept c, ArrayList typelabelvector ) {
    ...
    public static void linkTypeLabels( charger.obj.Graph h ) {
        ...
        while ( typelabels.hasNext() ) {
            ...
            craft.Craft.say( "looking for hypernyms of synset " + tsynset );
     ...
     */
    public static void linkHypernyms( TypeLabel t, Synset s, charger.obj.Graph h ) {
            ...
            if ( pointers.length == 0 ) {
                if ( !GEdge.areLinked( t, top ) ) {
                    if ( top.getOutermostGraph() == t.getOutermostGraph() ) {
                        ...
                        craft.Craft.say( "added link to top from " + t.getTypeLabel() );
                    }
                }
            } else {
                for ( int p = 0; p < pointers.length; p++ ) {
                    ...
                    for ( int supertypenum = 0; supertypenum < supertypes.length; supertypenum++ ) {
                        ...
                        h.insertInCharGerGraph( ge );
                        craft.Craft.say( "added link from " + t.getTypeLabel()
    ...
    public static Action makeTypeHierarchyAction = new AbstractAction() {
        ...
        public void actionPerformed( ActionEvent e ) {
            charger.obj.Graph g = makeTypeHierarchy();
            if ( g.getOwnerFrame() != null ) {
     ...
     */
    public static TypeLabel[] findBySynset( Synset s, charger.obj.Graph g ) {
