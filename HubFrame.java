...
//        }
        ...
        WebAddressButton.setFont( new java.awt.Font( "Arial", Font.PLAIN, 12 ) );
        ...
        totalMemory.setLocation( new java.awt.Point( 10, 140 ) );
        ...
        usedMemory.setLocation( new java.awt.Point( 10, 160 ) );
        ...
        freeMemory.setLocation( new java.awt.Point( 10, 180 ) );
...
//        for ( String filename : GraphDisplayList.getModel() ) {
    ...
    /**
     * Gets the names of any external modules. Looks first in each jar file then for a
     * regular set of classes in a class path. Only accepts names of classes that
     * end with "ModulePlugin.class". For each one, will create a menu item under
     * Tools that invokes the module's startup method.
     * @see charger.ModulePlugin
     */
    public void registerModulePlugins() {
        String[] modules = null;
...
//                    Global.warning( "can't open jarfile " + e.getMessage() );
                ...
                while ( entries.hasMoreElements() ) {
                    ...
                    if ( name.endsWith( Global.modulePluginClass )
                             && !name.contains( "/" + Global.modulePluginClass )) {
                        try {
                ...
                if ( moduleDirectoryFile.exists() ) {
                    ...
                    modules = moduleDirectoryFile.list( new FilenameFilter() {
                        public boolean accept( File f, String name ) {
                            if ( name.endsWith( Global.modulePluginClass ) && 
                                    !name.equalsIgnoreCase( Global.modulePluginClass ) ) {
                                return true;
                            }
                            return false;
                        }
